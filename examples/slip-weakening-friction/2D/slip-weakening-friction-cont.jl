...
function Residual(x, par)
    ...
    if t == 0.0
        return (f_fₚ .- τ₀_τₚ) / Δp_σ₀ .- (A * x[1:n]) / (abs(x[n+1]) * Δp_σ₀)
    else
        return (f_fₚ .- τ₀_τₚ) / Δp_σ₀ .- f_fₚ .* erfc.(abs.(x[n+1] * gc.x / t)) .- (A * x[1:n]) / (abs(x[n+1]) * Δp_σ₀)
    end
end
...
function main()
    ...
    # Parameters
    par = (t = 0.2, A = A, fᵣ_fₚ = fᵣ_fₚ, Δp_σ₀ = Δp_σ₀, τ₀_τₚ = τ₀_τₚ, n = n, gc = gc)
    ...
    filename = string(@__DIR__, "/slip-weakening-friction-cont.csv")
    ...
    function finaliseSolution(z, tau, step, br; kwargs...)
        ...
        open(string(@__DIR__, "/slip-weakening-friction-cont.csv"); append=true) do f
    ...
    br, x_final = BK.continuation(prob, PALC(tangent = Bordered()), opts_br; kwargsC...)
end
